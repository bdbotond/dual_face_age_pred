...
def img_trf_train(code,x1,y1,x2,y2):
    try:
...
def img_trf_val(code,x1,y1,x2,y2):
    try:
        original_img = cv2.cvtColor(cv2.imread(str(code)), cv2.COLOR_BGR2RGB)
        try:
            img=original_img[y1:y2, x1:x2, :]
...
class dataloader_train(torch.utils.data.Dataset):
    def __init__(self, frame):
        self.frame=frame
        self.front_path =frame['path'].tolist()
        self.ages = frame['age'].tolist()
        self.x1 = frame['x1'].tolist()
        self.y1 = frame['y1'].tolist()
        self.x2 = frame['x2'].tolist()
        self.y2 = frame['y2'].tolist()

    def __getitem__(self, index):
        img_conv=img_trf_train(str(self.front_path[index]),self.x1[index],self.y1[index],self.x2[index],self.y2[index])
...
class dataloader_test(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        img_conv=img_trf_val(str(self.front_path[index]),self.x1[index],self.y1[index],self.x2[index],self.y2[index])
        image, target = img_conv, self.ages[index]
        return image, target
...
data = pd.read_csv(str(sys.argv[2]))
